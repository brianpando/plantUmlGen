...
class PlantModels extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($classes as $class){
            //Feature @gabrielCapcha: Obteniendo los atributos de la clase
            ...
            foreach($model->fields as $i => $field){
                $t=explode(":", $field);
                $field_name=$t[0];
                $class_content .= "'$field_name',";
            }
            $class_content .= "\n\t];\n\n}";
            File::put($model_path."/$model->class_name.php",$class_content);
            $this->line("=> class $model->class_name created.");
        }
        ...
        foreach($relationships[0] as $relationship){
            ...
            switch($cardinality){
                case '11'://one to one
                    $relationshipcode1="\tpublic function ".strtolower($class2)."()\n\t{\n\t\treturn $"."this->hasOne('\App\\Models\\$class2');\n\t}";
                    $relationshipcode2="\tpublic function ".strtolower($class1)."()\n\t{\n\t\treturn $"."this->belongsTo('\App\\Models\\$class1');\n\t}";
                break;
                case '1*'://one to many
                    $relationshipcode1="\tpublic function ".strtolower($class2)."s()\n\t{\n\t\treturn $"."this->hasMany('\App\\Models\\$class2');\n\t}";
                    $relationshipcode2="\tpublic function ".strtolower($class1)."()\n\t{\n\t\treturn $"."this->belongsTo('\App\\Models\\$class1');\n\t}";

                break;

                case '**'://many to many
                    $relationshipcode1="\tpublic function ".strtolower($class2)."s()\n\t{\n\t\treturn $"."this->belongsToMany('\App\\Models\\$class2');\n\t}";
                    $relationshipcode2="\tpublic function ".strtolower($class1)."s()\n\t{\n\t\treturn $"."this->belongsToMany('\App\\Models\\$class1');\n\t}";
